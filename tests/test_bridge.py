...
class MessageBufferTests(unittest.TestCase):
    ...
    def test_windows_relative_attachment_is_resolved_under_attachment_root(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            message = {"id": "image", "type": 3, "filepath": relative_path.replace("/", "\\")}
            buffer.ingest(message)
            buffer.emit_ready(limit=10)

            self.assertEqual(buffer.pull(max_items=10, wait_ms=0)["messages"], [message])
    ...
    def test_missing_attachment_is_released_after_wait_timeout(self):
        ...
        message = {"id": "missing", "type": 3, "filepath": "missing\\image.dat"}
        buffer.ingest(message)
        buffer.emit_ready(limit=10)
